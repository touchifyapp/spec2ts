...
export function generateServers(file: ts.SourceFile, { servers }: OpenAPIObject, context: OApiGeneratorContext): ts.SourceFile {
    ...
    file = core.replaceSourceFileStatement(
        file,
        defaultsConst,
        core.updateVariableStatementPropertyValue(defaultsConst, "defaults", "baseUrl", defaultBaseUrl(servers)),
    );
...
export function generateDefaults(file: ts.SourceFile, context: OApiGeneratorContext): ts.SourceFile {
    if (context.options.importFetch) {
        ...
        file = core.replaceSourceFileStatement(
            file,
            defaultsConst,
            core.updateVariableStatementPropertyValue(defaultsConst, "defaults", "fetch", core.toExpression("fetch")),
        );
    }
...
export function generateFunctions(file: ts.SourceFile, spec: OpenAPIObject, context: OApiGeneratorContext): ts.SourceFile {
    const paths: typeof spec.paths = Object.fromEntries(
        Object.entries(spec.paths ?? {}).filter(([path]) => !context.options.prefix || path.startsWith(context.options.prefix)),
    );

    const functions: ts.FunctionDeclaration[] = Object.entries(paths)
        .map(([path, pathSpec]) => {
...
function generateFunction(
    ...
    const { name, query, header, paramsVars, args, bodyMode, bodyVar, response, responseVoid, responseJSON } = parseOperation(
        path,
        item,
        method,
        operation,
